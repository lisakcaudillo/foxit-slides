...
- Match the structure to the section's job: a comparison of options across shared DATA attributes with real values in a grid → combo-table-cols, a CONCEPTUAL two-way contrast (ideas, eras, styles, approaches — no tabular data) → combo-cards (two side-by-side) or combo-body (prose), ordered steps or a milestone schedule → combo-timeline, three-to-four parallel points → combo-cards, a section break → 08-divider, a numbered overview → 10-agenda, a memorable exact line from the source → combo-quote. A table is ONLY for real tabular data (comparable numbers/attributes sitting in cells), NEVER for a narrative or conceptual contrast — that reads as a spreadsheet where prose belongs.
...
- Choose each structure by the section's ACTUAL content, never for variety. combo-body is the correct choice for any general explanatory section — use it as often as the content calls for it, even on consecutive slides. Reach for a specialized structure (combo-cards, combo-table, combo-timeline, combo-stat, combo-metrics, 10-agenda) ONLY when the section genuinely has that shape: three-to-four parallel items, a data table of comparable values in cells (NOT a conceptual contrast — that is combo-cards or prose), real ordered steps or schedule, real numbers. Do NOT pick a specialized layout just to avoid repeating combo-body — a forced cards/stat slide on general content leaves slots empty and reads as broken. Variety is a RESULT of matching content, not a goal.${withImages ? `
